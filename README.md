# КБЖУ — Telegram Mini App

Личный счётчик калорий и КБЖУ в стиле Apple/iOS. Данные — в базе **Supabase**
(см. [SUPABASE.md](SUPABASE.md)), с фоллбэком на CloudStorage/localStorage.

## Возможности
- **Дневник** за день: кольцо прогресса ккал + полоски Б/Ж/У относительно цели.
- **Каталог**: плоский список позиций с КБЖУ на 100 г (ккал считается из Б/Ж/У).
- **Быстрая запись**: внести КБЖУ цифрами без выбора позиции.
- **Цель**: дневная норма ккал/белки/жиры/углеводы (ккал — из Б/Ж/У).

## Стек
Чистый HTML/CSS/JS (ES-модули), **без сборки и без зависимостей**. Системный шрифт
Apple (SF Pro). Деплоится как статика.

```
index.html        каркас + подключение Telegram SDK
styles.css        стили (Apple-минимализм, темизация под Telegram)
src/storage.js    CloudStorage ↔ localStorage
src/nutrition.js  расчёты КБЖУ (чистые функции)
src/state.js      in-memory store + сохранение
src/ui.js         экраны и нижние листы
src/app.js        точка входа
```

## Локальный запуск
```bash
cd "путь/к/kkal"
python3 -m http.server 8765
# открыть http://localhost:8765
```
Вне Telegram приложение работает на localStorage (данные только в этом браузере).

## Хостинг

Развёрнуто на **GitHub Pages**: https://aslanmazhidov.github.io/kkal/
Репозиторий: https://github.com/AslanMazhidov/kkal

**Обновление сайта** — обычный `git push`, Pages пересоберётся автоматически:
```bash
git add -A && git commit -m "..." && git push
```

## Подключение к боту (@BotFather)
Самый быстрый способ — кнопка меню:
1. Открыть **@BotFather** → `/newbot` (или взять существующего бота).
2. `/mybots` → выбрать бота → **Bot Settings → Menu Button → Configure menu button**.
3. Прислать URL: `https://aslanmazhidov.github.io/kkal/`, затем текст кнопки (напр. `КБЖУ`).
4. Открыть бота → нажать кнопку меню → приложение запустится внутри Telegram.
   Данные пойдут в **CloudStorage** и будут синхронизироваться между устройствами.

Альтернатива — `/newapp` (даёт прямую ссылку `t.me/<бот>/<имя>`): выбрать бота,
указать тот же Web App URL, имя, иконку 512×512.

## Заметки
- CloudStorage: ≤ 1024 ключей, значение ≤ 4096 байт. Данные разложены по ключам
  `food:<id>`, `recipe:<id>`, `day:<дата>`, `settings` — для личного использования
  лимитов хватает с запасом.
